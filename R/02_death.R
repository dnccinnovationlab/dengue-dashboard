...
raw_data<-read_sheet("164-CIeR5mSutgIUPVS2EteQy1lGMSEzFhHj7iCLqzqg",sheet="death")%>%
...
tmp <- zone_death %>%
  ...
  mutate(area=area.y)%>%
  mutate(Total_death=ifelse(is.na(Total)==TRUE,0,Total))%>%
  mutate(Zone=as.character(Zone),
         Ward=as.character(Ward),
         wardf=as.character(wardf))%>%
  as.data.frame()%>%
  dplyr::select(Date,week,date_range,week_ju,month,year,Zone,Ward,Address1,zone,ward,wardf,wardfc,area,coord.x,coord.y,Total_death)

OUTPUT_SHEET_ID  <- "1EMva389XPbDyJs80M3FZhgj8hGNCOouWzo4bz0cr0Ww"
OUTPUT_SHEET_TAB <- "dengue_death_output"
...
start_date=last_updated_data+1 
